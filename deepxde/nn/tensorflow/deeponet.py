...
class DeepONet(NN):
    ...
    def __init__(
        ...
        if callable(layer_sizes_branch[1]):
            ...
            if self.cnn:
                ...
                self.branch = FNN(layer_sizes_branch[:-1], activation_branch, kernel_initializer)

        self.combine = tf.keras.layers.Dense(layer_sizes_branch[-1])
    ...
    def call(self, inputs, training=False):
        ...
        if self.cnn:
            x_func = inputs[0][:,:-2]
            x_inv = inputs[0][:,-2:]
            x_func = self.branch(tf.expand_dims(x_func,-1))
            x_func = tf.concat([x_func,x_inv],axis=1)
            x_func = self.combine(x_func)
        else:
            x_func = inputs[0]
...
class DeepONetFunctional(NN):
    ...
    def __init__(
        ...
        if self.cnn:
            self.branch = tf.keras.Sequential([
                tf.keras.layers.Conv1D(filters=layer_sizes_branch[0],kernel_size=3,activation='relu'),
                tf.keras.layers.MaxPooling1D(pool_size=2,strides=2),
                tf.keras.layers.Conv1D(filters=layer_sizes_branch[1],kernel_size=3,activation='relu'),
                tf.keras.layers.MaxPooling1D(pool_size=2,strides=2),
                tf.keras.layers.Flatten()
                ])
        else:
        ...
        self.combine = tf.keras.Sequential([
            ...
            tf.keras.layers.Dense(layer_sizes_branch[-1])
        ])
        self.trunk = FNN(layer_sizes_trunk, self.activation_trunk, kernel_initializer)
        self.b = tf.Variable(tf.zeros(1, dtype=config.real(tf)))
    ...
    def call(self, inputs, training=False):
        ...
        if self.cnn:
            #x_func = inputs[0][:,:,:-1]
            #x_inv = tf.expand_dims(inputs[0][:,0,-1],-1)
            x_func = inputs[0]
            x_func = self.branch(x_func)
            #x_func = tf.concat([x_func,x_inv],axis=1)
            x_func = self.combine(x_func)
            ...
            x_func = inputs[0]
            m = tf.shape(x_func)[1]
            x_func = tf.reshape(x_func, [tf.shape(x_func)[0], -1])
            x_func = x_func[:,:m+1]
            x_func = self.branch(x_func)
