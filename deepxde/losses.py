...
def custom_loss(y_true,y_pred):
    ...
    l1 = bkd.reduce_mean(tf.multiply(tf.multiply(tf.exp(y_pred),y_true[2]),y_true[1])) # loss component 1
    l2 = -bkd.reduce_mean(tf.multiply(tf.multiply(y_pred,y_true[0]),y_true[1])) # loss component 2

    return l1+l2

def custom_loss_discrete(y_true,y_pred):

    l1 = bkd.reduce_mean(tf.multiply(tf.math.log(tf.exp(y_pred)+1),y_true[1])) # loss component 1
    l2 = -bkd.reduce_mean(tf.multiply(tf.multiply(y_pred,y_true[0]),y_true[1])) # loss component 2
...
LOSS_DICT = {
    ...
    "zero": zero,
    "custom": custom_loss,
    "custom_discrete": custom_loss_discrete
}
